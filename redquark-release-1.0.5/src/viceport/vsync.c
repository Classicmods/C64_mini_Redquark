...
{
    //refresh_frequency = 50; // Comes in at 50.124542 normally
...
//printf("vsync_do_vsync: sound_delay=%f  frame_ticks=%d  delay=%d\n", sound_delay, frame_ticks, delay);

#if (defined(HAVE_OPENGL_SYNC)) && !defined(USE_SDLUI)
    //refresh_cmp = (float)(c->refreshrate / refresh_frequency); FIXME
    refresh_cmp = (float)(50.0 / refresh_frequency);
    refresh_div = (int)(refresh_cmp + 0.5f);
    refresh_cmp /= (float)refresh_div;
//printf("vsync_do_vsync: refresh_cmp %f  refresh_div %d  refresh_frequency %lf   vsyncarch_freq %ld\n", refresh_cmp, refresh_div, refresh_frequency, vsyncarch_freq);

    if ((timer_speed == 100) && (!warp_mode_enabled) &&
        vsyncarch_vbl_sync_enabled() &&
        (refresh_cmp <= 1.02f) && (refresh_cmp > 0.98f) &&
        (refresh_div == 1)) {
        // vsyncarch_verticalblank(c, c->refreshrate, refresh_div); FIXME

vsyncarch_verticalblank(c, 50.0, refresh_div);
...
void vsyncarch_verticalblank(video_canvas_t *c, float rate, int frames)
{
    ...
    max = (frm * 7) >> 3;
    vbl = 0;
//printf( "max %lu  => %lu  (nowi %lu  lastx %lu)\n", max, (nowi - lastx), nowi, lastx );
